...
resource "aws_nat_gateway" "default" {
  ...
  tags = merge(
    var.tags, { "Name" = "${var.name}-${local.az_ids[count.index]}" }
  )
}
...
resource "aws_subnet" "public" {
  ...
  tags = merge(
    { "Name" = "public-${var.name}-${local.az_ids[count.index]}" },
...
resource "aws_subnet" "private" {
  ...
  tags = merge(
    { "Name" = "private-${var.name}-${local.az_ids[count.index]}" },
...
resource "aws_route_table" "public" {
  count  = min(local.public_subnets, 1)
  vpc_id = aws_vpc.default.id
  tags   = merge(var.tags, { "Name" = "public-${var.name}" })
}
...
resource "aws_route_table" "private" {
  ...
  tags = merge(
    var.tags, { "Name" = "private-${var.name}-${local.az_ids[count.index]}" }
  )
}
